...
const airtable = new airtableClient.AirtableClient(process.env.AIRTABLE_APP_ID, process.env.AIRTABLE_TABLE_ID, process.env.AIRTABLE_API_KEY);
...
module.exports = {
    async send(req, res) {
        ...
        form.parse(req, async (err, fields) => {
            ...
            if (!subject || !firstname || !lastname || !email) {
                logger.error("Missing data: Requires subject, firstname, lastname and email");
                res.sendStatus(500);
                return;
            }
    ...
    async sendJobApplication(req, res) {
        ...
        form.parse(req, async (err, fields, files) => {
            ...
            //send to airtable
            await airtable.sendEmployeeInformationToAirtable(name, email, cv, subject, FILE_DIRNAME, logger);
