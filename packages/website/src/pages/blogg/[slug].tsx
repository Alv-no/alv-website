...
import { MobileHeader, Footer, SEO, AlsoRead, SocialShare, createSlugForEmployee } from 'shared-components';
...
const ArticleTemplate : React.FC<Props> = ({ serverData: { article, articles } }) => {
  ...
  const wrappedArticles = {
    edges: articles.map((article : any) => ({ node: article })),
  };
  ...
  const socialTags = (tags && tags.map((tag : any) => tag.tag)) || '';
  const metaImage =
          ...
          >
            ...
            {mainImage && (
              <div className="w-full mb-3">
                <span
                  className={`relative z-0 fixed opacity-90`}
                >
                  <GatsbyImage
...
export async function getBlogArticleServerSide(slug : any) {
...
export async function getServerData(props : any) {
  ...
  try {
    ...
    const dedupeMap = new Map(
      articles.articles.map((item : any) => [item.slug.current, item])
    );
